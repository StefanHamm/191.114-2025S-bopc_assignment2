...
block_sizes = [(6,16)] # Fixed for this run
...
df_block_analysis.reset_index().to_csv('block_size_analysis2.csv', index=False)
...
# This assumes 'res_x' correctly represents the input problem size.
df_plot_data = df_raw.groupby('res_x').agg(
    mean_runtime=('runtime', 'mean'),
    max_runtime=('runtime', 'max')
)
...
if len(block_sizes) == 1:
    plot_title += f' (Block: {block_sizes[0][0]}x{block_sizes[0][1]})'
